...
- `GET /api/products/hot`：匿名查询热门商品，按销量倒序返回默认前 10 个商品。
...
阶段 2 接口已完成 MVP。阶段 3 已在 Service 层接入商品详情和热门商品 Redis 缓存，现有商品查询接口保持兼容；后续订单迭代再补充支付、取消和库存归还。
...
截至 2026-07-23，认证权限、商品分类、商品管理、商品目录和订单管理接口已落地，Java 后端基础功能阶段完成；商品目录新增热门商品查询接口，可作为 Redis 阶段和后续秒杀接口的接口基线。
