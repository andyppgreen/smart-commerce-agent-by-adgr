...
认证权限、商品分类、商品管理和订单管理 MVP 已完成，Java 后端基础功能阶段已通过验收。
...
完成阶段 2 文档收尾，将 `feature/java-backend` 合并到 `dev`，再进入 Redis 缓存能力阶段。
...
- 应用上下文和健康接口测试使用测试资源中的 H2 配置。
- 登录控制器测试使用 Web 切片和 Mockito，登录服务测试使用 Mockito 单元测试。
- 认证、权限、分类、商品目录、商品管理和订单集成测试使用 `local` 配置连接真实 MySQL。
- `git diff --check` 已通过，阶段 2 业务代码已推送到 `origin/feature/java-backend`。

验收覆盖：

- JWT 登录、缺少 Token 拒绝访问和当前用户解析。
- `ADMIN` 与 `CUSTOMER` 的接口角色隔离。
- 商品分类和商品 CRUD、逻辑删除、分页筛选和商品编码唯一性。
- 匿名商品目录只返回已上架商品，并隐藏后台字段和精确库存。
- 多商品下单、后端金额计算、商品快照、事务内库存扣减。
- 库存不足时订单和明细不落库，商品库存保持不变。
- 买家只能查询自己的订单，管理员可以查询任意订单。

## 运行入口

- 模块运行、配置和测试说明：`backend/smart-commerce-admin/README.md`。
- API 清单：`docs/03-api-design.md`。
- 数据库结构与迁移说明：`docs/02-database-design.md`。
- Redis 和秒杀下一阶段：`docs/05-redis-seckill.md`。
...
- Token 刷新、主动退出和失效名单。
- 支付、订单取消、超时关单和库存归还。
- Redis 缓存、限流、秒杀和 RabbitMQ 异步下单。
- OpenAPI 文档、生产密钥管理和容器化部署。

这些内容应在后续阶段按业务需要实现，不应被误判为当前代码中已经存在。
...
1. 将 `feature/java-backend` 合并并推送到 `dev`。
2. 从更新后的 `dev` 创建 `feature/redis-seckill`。
3. 阅读 `docs/05-redis-seckill.md` 并确认 Redis 第一阶段范围。
4. 接入 Redis，先实现商品详情缓存。
5. 增加缓存命中、失效和穿透处理测试。
...
截至 2026-07-15，Java 后端已完成认证权限、商品分类、商品管理和订单管理 MVP，21 项测试全部通过，阶段 2 验收完成。下一步先合并 `dev`，再从更新后的 `dev` 开始 Redis 缓存能力。
